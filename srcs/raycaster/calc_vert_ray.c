...
/*   Updated: 2022/07/19 01:02:31 by jrasser          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	ft_dist_sur_y(t_data *d, double angle)
{
	...
	posX_block = (int)d->ray_data.pos_x % BLOCK_SIZE;
	//printf("pos x block : %d\n", posX_block);
	...
	if (sens_vert == 1)
	{
		y0 = (BLOCK_SIZE - posX_block) * (tan(ft_conv_in_rad(angle)));
		y1 = ((BLOCK_SIZE - posX_block) + BLOCK_SIZE) * tan(ft_conv_in_rad(angle));
	}
	else
	{
		y0 = (posX_block) * (tan(ft_conv_in_rad(angle)));
		y1 = (posX_block + BLOCK_SIZE) * tan(ft_conv_in_rad(angle));
	}
	...
	//x_test = d->ray_data.pos_x + (sens_vert * (BLOCK_SIZE - posX_block));
	...
	{
		x_test = d->ray_data.pos_x + (sens_vert * (posX_block));
		y_test = d->ray_data.pos_y - (sens_vert * y0);
	}
	...
	while (ft_block_is_wall_on_vert_next_x(d, x_test, y_test, sens_vert) != 1)
	{
		...
		if (sens_vert == 1)
		{
			x_test = d->ray_data.pos_x + (sens_vert * ((BLOCK_SIZE - posX_block) + (j * BLOCK_SIZE)));
			y_test = d->ray_data.pos_y - ((sens_vert) * (y0 + (j * y)));
		}
		else
		{
			x_test = d->ray_data.pos_x + (sens_vert * ((posX_block) + (j * BLOCK_SIZE)));
			y_test = d->ray_data.pos_y - ((sens_vert) * (y0 + (j * y)));
		}
